...
while($row = $result_profile->fetch_array(MYSQLI_ASSOC))
{
	...
	$age = $row['age'];
	$gender = $row['gender'];
	echo "<div id='infowindow-div'><div class='user-profile'><span class='name'>$name</span><span class='age'>$age</span><span class='gender'>$gender</span><span class='hometown'>$hometown</span></div>";
}
while($row = $result_work->fetch_array(MYSQLI_ASSOC))
{
	$organization = $row['organization'];
	$job_profile = $row['job_profile'];
	echo "<div class='job-profile'>Working at <span class='organization'>$organization</span> as <span class='designation'>$job_profile</span></div>";
}
while($row = $result_interest->fetch_array(MYSQLI_ASSOC))
{
	...
	$i4 = $row['i4'];
	$i5 = $row['i5'];
	echo "<div class='user-interests'>Likes $i1, $i2, $i3, $i4, $i5</div></div>";
}
?>
